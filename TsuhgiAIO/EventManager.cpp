...
namespace eventmanager
{
	...
	*/
	...
	static std::vector<std::function<bool(event_id_t, IUnit *, std::string const)>> pauseAnimationHandlers;
	...
	event_id_t UnitEventManager::RegisterPauseAnimationEvent(std::function<bool(event_id_t, IUnit *, std::string const)> func)
	{
	...
	PLUGIN_EVENT(bool) OnUnitPauseAnimation(IUnit *source, std::string const animationId)
	{
		...
		for (event_id_t i = 0; i < pauseAnimationHandlers.size(); i++)
		{
			...
			{
				auto ret = pauseAnimationHandlers[i](i, source, animationId);
